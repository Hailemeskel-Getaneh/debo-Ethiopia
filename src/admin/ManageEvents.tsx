  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      start_date:
        ...
        "T" +
        ((formData.get("time") as string) || "00:00") +
        ":00",
      end_date:
        (formData.get("date") as string) +
        "T" +
        ((formData.get("time") as string) || "23:59") +
        ":00",
      program_id: formData.get("program_id")
        ? Number(formData.get("program_id"))
        : null,
    };
    ...
    try {
      ...
      // Invalidate cache after any create/update
      invalidateCache("events");
      ...
      if (modalMode === "Add") {
        ...
        setEvents(
          events.map((ev) => (ev.id === eventResult.id ? eventResult : ev)),
        );
      }
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-100 dark:border-zinc-800 text-center">
          ...
          <p className="text-zinc-500 dark:text-zinc-400 mt-1 max-w-xs mx-auto">
            {searchTerm
              ? `No events match "${searchTerm}" and status "${filterStatus}"`
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  <div className="flex justify-between items-start">
                    ...
                    <span
                      className={`hidden sm:inline-flex px-2.5 py-1 rounded-full text-xs font-semibold ${statusStyles[status]}`}
                    >
                      {status}
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="md:col-span-2 space-y-2">
                    ...
                    <input
                      ...
                      required
                      placeholder="e.g. Community Health Walk"
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      required
                      placeholder="e.g. Unity Park, Addis Ababa"
                      className="w-full px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 outline-none"
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <select
                      name="program_id"
                      defaultValue={selectedEvent?.program_id || ""}
                      className="w-full px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 outline-none"
                    >
