...
const ManageNews: React.FC = () => {
  ...
  const [filterTab, setFilterTab] = useState<"All" | "published" | "draft">(
    "All",
  );
  ...
  const handleDelete = async () => {
    if (!selected) return;
    try {
      await newsService.delete(selected.id);
      invalidateCache("news");
  ...
  const coverImage = (a: NewsArticle) =>
    a.images.length > 0 ? a.images[0].image : null;
    ...
    <div className="space-y-6">
      ...
      {error && (
        <div className="flex items-center gap-3 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 rounded-xl text-red-700 dark:text-red-300">
          ...
          <button onClick={fetchArticles} className="ml-auto text-xs underline">
            Retry
          </button>
        </div>
      )}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-zinc-50/50 dark:bg-zinc-800/50 border-b border-zinc-100 dark:border-zinc-800">
                {["Article", "Author", "Status", "Date", "Actions"].map(
                  (h, i) => (
                    <th
                      key={h}
                      className={`px-6 py-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider ${i === 4 ? "text-right" : ""}`}
                    >
                      {h}
                    </th>
                  ),
                    ...
                    >
                      <td className="px-6 py-4 min-w-[280px]">
                        <div className="flex items-center gap-3">
                          ...
                          <p className="font-semibold text-zinc-900 dark:text-zinc-100 text-sm line-clamp-2">
                            {article.title}
                          </p>
                        </div>
                      </td>
        ...
        <div className="px-6 py-4 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={page <= 1}
              className="px-4 py-2 border border-zinc-200 dark:border-zinc-800 rounded-lg text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800 disabled:opacity-40"
            >
              Previous
            </button>
            <button
              ...
              className="px-4 py-2 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 rounded-lg text-sm hover:bg-zinc-800 disabled:opacity-40"
            >
              Next
            </button>
          </div>
              ...
              <div className="flex items-center justify-between p-6 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-zinc-500">
                      Draft or publish a news article.
                    </p>
                  </div>
                </div>
                <button
              ...
              >
                      ...
                      {imageFile ? (
                        ...
                        <>
                          ...
                          <div className="flex text-sm text-zinc-600 dark:text-zinc-400">
                            <span className="relative cursor-pointer font-bold text-primary-600">
                              Upload a file
                            </span>
                ...
                <div className="flex gap-4 pt-4">
                  ...
                  <button
                    type="submit"
                    disabled={saving}
                    className="flex-1 px-6 py-3 rounded-xl bg-primary-600 text-white font-bold hover:bg-primary-700 shadow-lg transition-all disabled:opacity-60 flex items-center justify-center gap-2"
                  >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex gap-4">
                <button
                  onClick={() => setIsDeleteOpen(false)}
                  className="flex-1 px-6 py-3 rounded-xl border font-bold hover:bg-zinc-50 transition-colors"
                >
                  Cancel
                </button>
                <button
                  onClick={handleDelete}
                  className="flex-1 px-6 py-3 rounded-xl bg-red-600 text-white font-bold hover:bg-red-700"
                >
                  Delete
                </button>
              </div>
