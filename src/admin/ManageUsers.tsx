...
const ManageUsers: React.FC = () => {
  ...
  const getRoleLabel = (user: User) => {
    ...
    const rObj = roles.find(
      (r) =>
        String(r.id) === String(user.role) ||
        r.name.toUpperCase() === roleIdOrName,
    );
  ...
  const filteredUsers = users.filter((user) => {
    ...
    return (
      label === selectedRoleFilter.toUpperCase() ||
      String(user.role).toUpperCase() === selectedRoleFilter.toUpperCase()
    );
  });
  ...
  const getRoleBadgeClass = (role?: string | number, isSuperuser?: boolean) => {
    ...
    if (r === "EXECUTIVE_DIRECTOR" || r === "ADMIN")
      return "bg-purple-50 text-purple-700 border-purple-200";
    if (r === "PROGRAM_COORDINATOR")
  ...
  const uniqueRoleFilters = [
    "All",
    ...Array.from(new Set(users.map((u) => getRoleLabel(u)))),
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-zinc-900 p-6 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800">
        <div>
          <h2 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100 italic">
            User Management
          </h2>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 font-medium">
            Manage access, roles, and privileges for all users.
          </p>
        </div>
        {isSuperAdmin && (
          <button
              ...
              {loading ? (
                ...
                filteredUsers.map((user, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-3 py-1 text-[10px] font-bold rounded-full border uppercase tracking-wider ${getRoleBadgeClass(user.role, user.is_superuser)}`}
                      >
        ...
        <div className="px-6 py-4 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-4 py-2 border rounded-xl text-xs font-bold disabled:opacity-30 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
            >
              Previous
            </button>
            <button
              ...
              onClick={() => setPage((p) => p + 1)}
              className="px-4 py-2 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 rounded-xl text-xs font-bold disabled:opacity-30 transition-colors"
            >
              Next
            </button>
          </div>
              ...
              <form className="p-6 space-y-5" onSubmit={handleSave}>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 focus:outline-none focus:ring-2 focus:ring-primary-500/30 focus:border-primary-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider">
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="flex-1 px-6 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 font-bold text-xs uppercase tracking-wider hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={saving}
                    className="flex-1 px-6 py-3 rounded-xl bg-primary-600 text-white font-bold text-xs uppercase tracking-wider hover:bg-primary-700 disabled:opacity-50 transition-all"
                  >
                    {saving ? (
        ...
        {isDeleteOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              exit={{ opacity: 0, scale: 0.95 }}
              className="relative w-full max-w-sm bg-white dark:bg-zinc-900 rounded-3xl shadow-2xl p-8 text-center border border-zinc-100 dark:border-zinc-800"
            >
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => setIsDeleteOpen(false)}
                  className="flex-1 px-6 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 font-bold text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
                >
                  Cancel
                </button>
                <button
                  onClick={handleDelete}
                  className="flex-1 px-6 py-3 rounded-xl bg-red-600 text-white font-bold text-sm hover:bg-red-700 transition-all"
                >
                  Delete
                </button>
              </div>
